...
class LocalPlaylistGenerator:
    ...
    def __init__(self, db_path: str = "music_library.db", model_name: str = "nomic-embed-text:latest"):
        """
        ...
        self.track_embedder = LocalTrackEmbedder(db_path, embedding_service=self.embedding_service)
        
    def generate_playlist(self, query: str, max_tracks: int = 20, 
        ...
        try:
            ...
            if extracted_count:
                ...
                logger.info(f"🔢 Found track count in query, setting max_tracks to {max_tracks}")
            ...
            if artist:
                ...
                results = [track for track in results if artist.lower() in track.get('artist', '').lower()]
            ...
            logger.info(f"✅ Generated playlist with {len(filtered_results)} tracks in {generation_time:.2f}s")
    ...
    def _apply_genre_filtering(self, query: str, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        if not genre_keywords:
            logger.info("🎵 No specific genre detected in query, using semantic similarity only")
            return results
        ...
        for track in results:
            ...
            if genre_match:
                ...
                boosted_score = min(1.0, similarity_score + 0.1)  # Boost by 0.1, cap at 1.0
        ...
        max_other_tracks = max(5, len(genre_matches) // 2)  # At most half as many non-genre tracks
        combined_results = genre_matches + other_tracks[:max_other_tracks]
        
        logger.info(f"🎵 Genre filtering: {len(genre_matches)} genre matches, {len(other_tracks[:max_other_tracks])} other tracks")
    ...
    def _extract_genre_keywords(self, query: str) -> List[str]:
        ...
        genre_mapping = {
            ...
            'rock': ['rock', 'hard rock', 'soft rock', 'classic rock', 'punk rock', 'indie rock'],
            'pop': ['pop', 'pop rock', 'synth pop', 'indie pop'],
            'hip hop': ['hip hop', 'rap', 'trap', 'r&b', 'soul'],
            'electronic': ['electronic', 'edm', 'techno', 'house', 'trance', 'ambient', 'dubstep'],
    ...
    def _filter_and_deduplicate_results(self, results: List[Dict[str, Any]], 
                                       min_similarity: float, max_tracks: int) -> List[Dict[str, Any]]:
        """Filter results by similarity and remove duplicates using multiple strategies"""
        # Filter by similarity threshold
        filtered = [track for track in results if track.get('similarity_score', 0) >= min_similarity]
        
        logger.info(f"🔍 Starting deduplication on {len(filtered)} tracks (similarity ≥ {min_similarity})")
        ...
        logger.info(f"📍 Location deduplication: {len(filtered)} → {len(location_deduplicated)} tracks")
        ...
        logger.info(f"🎵 Title/Artist deduplication: {len(location_deduplicated)} → {len(combination_deduplicated)} tracks")
        ...
        logger.info(f"🆔 Track ID deduplication: {len(combination_deduplicated)} → {len(final_deduplicated)} tracks")
        ...
        logger.info(f"🧠 Smart title deduplication: {len(final_deduplicated)} → {len(smart_deduplicated)} tracks")
        
        # Limit to max_tracks
        final_results = smart_deduplicated[:max_tracks]
        
        logger.info(f"✅ Final deduplication summary: {len(results)} → {len(final_results)} tracks")
        
        return final_results
    
    def _smart_title_deduplication(self, tracks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
                ...
                if base_title and base_title not in processed_titles:
                    ...
                    for other_track in artist_tracks:
                        ...
                        if other_base == base_title:
                            if other_track.get('similarity_score', 0) > best_track.get('similarity_score', 0):
    ...
    def _extract_base_title(self, title: str) -> str:
        ...
        suffixes_to_remove = [
            ' (remix)', ' (remastered)', ' (live)', ' (acoustic)', 
            ' (radio edit)', ' (extended)', ' (clean)', ' (explicit)',
            ' (original mix)', ' (club mix)', ' (dub mix)',
            ' - remix', ' - remastered', ' - live', ' - acoustic'
        ]
    ...
    def save_playlist_m3u(self, tracks: List[Dict[str, Any]], query: str, 
                         output_dir: str = "playlists") -> str:
        """
    ...
    def print_playlist_summary(self, tracks: List[Dict[str, Any]], query: str):
        ...
        if tracks:
            avg_similarity = sum(t.get('similarity_score', 0) for t in tracks) / len(tracks)
                ...
                if save in ['y', 'yes']:
                    ...
                    if open_music in ['y', 'yes']:
                        try:
                            ...
                            print("💡 You can open it manually with: python open_in_apple_music.py")
