...
class TestCoreFunctionality(unittest.TestCase):
    ...
    def test_track_embedding(self, mock_ollama: Mock) -> None:
        ...
        mock_embedding: np.ndarray = np.random.rand(384).astype(
            np.float32
    ...
    def test_playlist_generation(self, mock_ollama: Mock) -> None:
        ...
        # Mock the embeddings response
        mock_embedding: np.ndarray = np.random.rand(384).astype(np.float32)
        mock_client.embeddings.return_value = {"embedding": mock_embedding.tolist()}
        ...
        self.assertEqual(playlist[0]["name"], "Test Song 1")
        self.assertEqual(playlist[1]["name"], "Test Song 3")
        self.assertEqual(playlist[2]["name"], "Test Song 2")
