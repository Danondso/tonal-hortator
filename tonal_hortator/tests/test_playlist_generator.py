...
class TestLocalPlaylistGenerator(unittest.TestCase):
    """Test playlist generator functionality"""
    ...
    def setUp(self) -> None:
        ...
        self.generator = LocalPlaylistGenerator(self.temp_db.name)
    ...
    def test_generate_playlist_success(self) -> None:
        ...
        # Generate playlist
        result = self.generator.generate_playlist("test query", max_tracks=3)
    ...
    def test_generate_playlist_vague_query(self) -> None:
        ...
        # Generate playlist
        result = self.generator.generate_playlist("test query", max_tracks=2)
    ...
    def test_generate_playlist_with_custom_max_tracks(self) -> None:
        """Test playlist generation with custom max_tracks parameter"""
        # Mock embeddings and track data
        mock_embeddings = [[0.1] * 384] * 5
        mock_track_data = [
            {"id": i, "name": f"Track {i}", "artist": f"Artist {i}"} for i in range(5)
        ...
        result = self.generator.generate_playlist("test query", max_tracks=5)
        ...
        self.mock_embedding_service.similarity_search.assert_called_once_with(
            "test query", mock_embeddings, mock_track_data, top_k=25
        )
