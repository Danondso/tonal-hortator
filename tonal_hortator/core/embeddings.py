...
class OllamaEmbeddingService:
    ...
    def __init__(self, model_name: str = "nomic-embed-text:latest", host: Optional[str] = None):
        """
    ...
    def _test_connection(self):
        ...
        try:
            ...
            if self.model_name in model_names:
                logger.info(f"✅ Connected to Ollama and found model: {self.model_name}")
            else:
                logger.warning(f"⚠️  Model {self.model_name} not found in Ollama. Available models: {model_names}")
    ...
    def get_embedding(self, text: str) -> np.ndarray:
        ...
        try:
            ...
            logger.debug(f"Generated embedding in {embedding_time:.3f}s (dim: {len(embedding)})")
            return embedding
        ...
        for i in range(0, len(texts), batch_size):
            ...
            try:
                ...
                if len(batch_embeddings) == len(batch_texts):
                    ...
                    logger.error(f"❌ Mismatched embedding count. Got {len(batch_embeddings)} for {len(batch_texts)} prompts.")
                    raise Exception("Incomplete batch response from Ollama library")

            except Exception as e:
                logger.error(f"❌ Batch embedding request failed for batch {batch_num}: {e}")
                logger.info("🔄 Falling back to individual embeddings for this batch (redundant, but safe)...")
                for text in batch_texts:
                    try:
    ...
    def similarity_search(self, query: str, embeddings: List[np.ndarray], 
                          track_data: List[Dict[str, Any]], top_k: int = 50) -> List[Dict[str, Any]]:
        """
